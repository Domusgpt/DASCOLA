/**
 * Viking Village Fleet Map — Currents Layer
 * ===========================================
 * Particle-based ocean current animation for the NJ Atlantic waters.
 *
 * Canvas: fleetCanvasCurrents (z-index: 2)
 * Redraws every frame at 60fps.
 */
...
export function initParticles(config) {
  ...
  for (var i = 0; i < count; i++) {
    // Use placeholder dimensions; actual positions assigned on first draw
    particles[i] = spawnParticle(800, 600);
  }
...
export function drawCurrents(ctx, cm, particles, currentData, config, t) {
  ...
  // Pre-project all current segment endpoints
  var segs = [];
  var ci, si, cur, cPts, segP1, segP2;
